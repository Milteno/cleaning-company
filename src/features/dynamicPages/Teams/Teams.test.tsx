...
describe('Loading spinner', () => {
  it('should be visible', () => {
    ...
    act(() => {
      ...
      ReactDOM.render(renderedComponent(initialState, "/zespoly"), node)
    })
...
describe('Error handling', () => {
  ...
  it('should show message about empty data array', async () => {
    ...
    expect(mockedFetch).toBeCalledTimes(1);
    expect(mRes.json).toBeCalledTimes(1);
    expect(message?.textContent).toContain("Zespoły pracowników");
  });

  it('should show data in array', async () => {
    const response: any[] = [{ "_id": "61db1b5f05e3945690bdcf1a", "name": "Pracusie", "employee_id": ["id"] }];
    ...
    expect(mockedFetch).toBeCalledTimes(1);
    expect(mRes.json).toBeCalledTimes(1);
    expect(message?.textContent).toContain("Pracusie");
  });
});
